...
class Auth extends CI_Controller
{
	function __construct()
	{
		...
		$this->lang->load('tank_auth','portugues');
	}
	...
	function login()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$this->load->view(base_cms().'login', $data);
		}
	}
	 ...
	 function register()
	{
		if (!$this->tank_auth->is_logged_in()) {									// logged in
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
						//$use_username ? $this->form_validation->set_value('username') : '',
						$use_username ? $this->form_validation->set_value('username') : $usuario[0],
						$this->form_validation->set_value('email'),
						$this->form_validation->set_value('password'),
						$email_activation))) {									// success
	...
	function forgot_password()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$this->form_validation->set_rules('login', 'E-mail', 'trim|required|xss_clean');
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->forgot_password(
					...
					$this->_show_message($this->lang->line('auth_message_new_password_sent').' '.anchor('/cms/', 'Página Inicial'));
					return;
			...
			$this->load->view('cms/login_esqueci_senha', $data);
		}
	}
	...
	function reset_password($user_id, $new_pass_key	)
	{
		...
		if ($this->form_validation->run()) {								// validation ok
			if (!is_null($data = $this->tank_auth->reset_password(
				...
				$this->_show_message($this->lang->line('auth_message_new_password_activated').' '.anchor('/cms/', 'Login'));
				return;

			} else {														// fail
				$this->_show_message($this->lang->line('auth_message_new_password_failed').' '.anchor('/cms/', 'Página Inicial'));
				return;
			}
		} else {
		...
		$this->load->view('cms/login_ressetar_senha', $data);
	}
	...
	function _send_email($type, $email, &$data)
	{
		...
		$this->email->message($this->load->view('cms/email/'.$type.'-html', $data, TRUE));
		$this->email->set_alt_message($this->load->view('cms/email/'.$type.'-txt', $data, TRUE));
		$this->email->send();
	}
